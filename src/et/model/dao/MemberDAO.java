...
public class MemberDAO {
	 ...
	 * id 중복 체크
	 */
	public String idCheck(String memberId) throws SQLException{//id중복체크
	...
	public int insert(MemberDTO dto) throws SQLException{ //회원 가입
		PreparedStatement ps = null;
		  Connection con = null;
		  int result=0;
		  try {
		   ...
		   ps.setString(5, dto.getMemberGender());
		   ps.setString(6, dto.getMemberPhone());
		   ps.setString(7, dto.getMemberEmail());
		   ps.setString(8, dto.getMemberQuestion());
		   ps.setString(9, dto.getMemberAnswer());
	 ...
	 * 로그인 체크
	 */
	public int loginCheck(String memberId, String memberPw) throws SQLException{ //db에 저장된 id와 pw를 체크한다
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	try{
		...
		if(rs.next()){ //입력된 아이디에 해당하는 비밀번호가 존재하는 경우
			...
			if(dbPW.equals(memberPw)) result=1; //넘겨받은 비번이 같음
			else result=0; //넘겨받은 비번 다름	
		}else{
